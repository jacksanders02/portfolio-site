...
export default function MazeGeneratorClient() {
  ...
  function resetCanvasSize(
    ...
    const widthLim: number = Math.round(window.screen.width * 0.8);
    const heightLim: number = Math.round(window.screen.height * 0.66);
  ...
  function initialiseMap(): void {
    if (canvasRef.current && widthInputRef.current && heightInputRef.current) {
      ...
      if (Number.isNaN(width + height) || width < 2 || height < 2) {
        setErrorMessage(
  ...
  function finishSelectedCallback(): void {
  ...
  function startSelectedCallback(): void {
    if (canvasRef.current) {
      setStage(3);
  ...
  function startSearch(): void {
    if (canvasRef.current) {
      ...
      maze!.map.forEach((row: AStarNode[]) => {
    ...
    <div className="flex flex-col flex-fill-space items-center gap-8">
      ...
      {stage === 0 && (
        <>
          ...
          <div
            className="w-11/12 md:w-2/3 flex gap-6 items-center justify-evenly"
          >
            <button
              id="generate-maze"
              type="button"

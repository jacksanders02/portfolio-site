    ...
    <main>
      <PageContainer>
        ...
        <p className={`text-center`}>
          ...
          captured his requirements in the form of story cards and mockup
          diagrams. The outline of the system is as follows: there are two main
        ...
        <p className={`text-center`}>
          ...
          are all styled with custom CSS to create the look of our application,
          using SCSS variables for all of the terrain colours, in order to keep
          a consistent look between what is displayed on the sidebar, and what
          is drawn on the map. Map creators can also change the
          &lsquo;tag&rsquo; of the map (for example, from &lsquo;In
          ...
          saved to the global &lsquo;courses&rsquo; database, but rather to the
        ...
        <p className={`text-center`}>
          ...
          how many strokes they would gain/lose if their shot followed the
          displayed white line. They can also toggle their shot dispersion for a
          specific club (the script for this was written by one of my
          teammates), to see whether their selected aiming point is realistic
          for them. The main part of this page, which was initially set by our
          client as a most-likely unreachable target, is the shot optimisation
          ...
          aiming point within a 150&deg; cone, centered on the hole (as it is
          highly unlikely that the optimal shot will be outside of that range,
          and limiting the search range speeds up the algorithm massively), and
          outputs the club/aim point combination that produces the best average
          strokes gained. The algorithm also takes into account trees - if the
          shot being checked starts or ends less than 20m from a tree that that
          the line of the shot intersects, it is assumed that that tree will be
          blocking the shot, and therefore that shot is discarded. However, this
          only works if a tree has been marked on the map - either by the map
          creators, or by the user themselves in their annotations. The 20m
          threshold is somewhat arbitrary - it is the distance that a ball would
          need to travel to clear a 20m high tree, assuming the ball is
          travelling at 45&deg;. This could be improved by allowing the map
          creator to change the height of the trees that they are marking, and
          then checking the shot line against individual tree heights, rather
          than assuming that all trees are 20m tall.
        </p>
      </PageContainer>
    </main>
  );
